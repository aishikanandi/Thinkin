      ...
      <Topbar isAuthenticated={isAuthenticated}/>
        ...
        <Route exact path='' element={ <Home posts={posts} cats={cats} isAuthenticated={isAuthenticated}/>}/>
          ...
          {posts.map((post, index) => {
            return <Route exact path={`/${post.title}`} element={<Single title={post.title} description ={post.description} date={post.date?post.date.slice(0,10):""} imageUrl={`http://localhost:5000/images/${post.image}`} comments={post.comments} id={post._id} />} key={index}  />;
          })}
